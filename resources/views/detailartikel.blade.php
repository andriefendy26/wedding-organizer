...
<div class="min-h-screen ">
    ...
    <div class="relative {{ $artikel->image_url ? 'bg-cover bg-center' : 'bg-gradient-to-br from-purple-600 to-pink-600' }} h-96 mt-24"
        ...
        <div class="absolute inset-0 flex items-center">
            <div class="container px-4 mx-auto md:px-32">
                <div class="max-w-4xl">
                    <nav class="flex items-center gap-2 mb-6 text-sm text-white/80">
                    ...
                    <div class="flex flex-wrap items-center gap-4 mb-4">
                        @if($artikel->formatted_tags)
                            @foreach(array_slice($artikel->formatted_tags, 0, 2) as $tag)
                                <span class="px-4 py-2 text-sm text-white rounded-full bg-white/20 backdrop-blur-sm">
                    ...
                    <h1 class="mb-4 text-3xl font-bold leading-tight text-white md:text-5xl edu-vic-wa-nt-hand">
                        {{ $artikel->judul }}
                    </h1>
                    
                    @if($artikel->sub_judul)
                        <p class="max-w-3xl text-lg md:text-xl text-white/90 pt-serif-regular-italic">
                            {{ $artikel->sub_judul }}
                        </p>
                    @endif
                </div>
        ...
        {{-- Decorative elements --}}
        <div class="absolute w-20 h-20 rounded-full top-10 right-10 bg-gradient-to-r from-yellow-500 to-orange-500 opacity-30"></div>
        <div class="absolute w-16 h-16 rounded-full bottom-10 left-10 bg-gradient-to-r from-blue-500 to-green-500 opacity-30"></div>
    </div>

    {{-- Article Content --}}
    <div class="container px-4 py-16 mx-auto md:px-32">
        <div class="max-w-4xl mx-auto">
            {{-- Author Info --}}
            <div class="flex items-center gap-4 pb-8 mb-8 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-r from-purple-500 to-pink-500">
                    <span class="text-xl font-bold text-white">{{ substr($artikel->author, 0, 1) }}</span>
                </div>
                <div>
                    <h3 class="text-lg font-semibold text-black dark:text-white poppins-regular">{{ $artikel->author }}</h3>
                    <p class="text-sm text-gray-600 dark:text-gray-300">Wedding Planner & Decorator Expert</p>
                </div>
                <div class="flex gap-3 ml-auto">
                    <button onclick="shareToSocial('twitter')" class="p-2 transition-colors bg-gray-100 rounded-full dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600">
                    ...
                    <button onclick="shareToSocial('facebook')" class="p-2 transition-colors bg-gray-100 rounded-full dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600">
                        <svg class="w-5 h-5 text-gray-600 dark:text-gray-300" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div class="prose prose-lg text-gray-700 max-w-none dark:text-gray-300">
                <div class="space-y-6 text-lg leading-relaxed pt-serif-regular article-content">
                    {!! $artikel->content !!}
                </div>
            </div>
            ...
            <div class="pt-8 mt-12 border-t border-gray-200 dark:border-gray-700">
                <div class="flex flex-col items-start justify-between gap-6 sm:flex-row">
                    @if($artikel->formatted_tags && count($artikel->formatted_tags) > 0)
                        <div>
                            <h3 class="mb-3 text-lg font-semibold text-black dark:text-white">Tags:</h3>
                    ...
                    <div>
                        <h3 class="mb-3 text-lg font-semibold text-black dark:text-white">Bagikan:</h3>
                        <div class="flex gap-3">
                            <button onclick="shareToSocial('twitter')" class="p-3 text-white transition-colors bg-blue-500 rounded-full hover:bg-blue-600">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"/>
                                </svg>
                            </button>
                            <button onclick="shareToSocial('facebook')" class="p-3 text-white transition-colors bg-blue-600 rounded-full hover:bg-blue-700">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M22.46 6c-.77.35-1.6.58-2.46.69.88-.53 1.56-1.37 1.88-2.38-.83.5-1.75.85-2.72 1.05C18.37 4.5 17.26 4 16 4c-2.35 0-4.27 1.92-4.27 4.29 0 .34.04.67.11.98C8.28 9.09 5.11 7.38 3 4.79c-.37.63-.58 1.37-.58 2.15 0 1.49.75 2.81 1.91 3.56-.71 0-1.37-.2-1.95-.5v.03c0 2.08 1.48 3.82 3.44 4.21a4.22 4.22 0 0 1-1.93.07 4.28 4.28 0 0 0 4 2.98 8.521 8.521 0 0 1-5.33 1.84c-.34 0-.68-.02-1.02-.06C3.44 20.29 5.7 21 8.12 21 16 21 20.33 14.46 20.33 8.79c0-.19 0-.37-.01-.56.84-.6 1.56-1.36 2.14-2.23z"/>
                                </svg>
                            </button>
                            <button onclick="shareToSocial('whatsapp')" class="p-3 text-white transition-colors bg-green-500 rounded-full hover:bg-green-600">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    @if($relatedArtikels->count() > 0)
        <div class="py-16 bg-gray-50 dark:bg-gray-900">
            <div class="container px-4 mx-auto md:px-32">
                <h2 class="mb-12 text-4xl font-semibold text-center text-black edu-vic-wa-nt-hand dark:text-white">
                    Artikel Terkait
                </h2>
                
                <div class="grid max-w-6xl grid-cols-1 gap-8 mx-auto md:grid-cols-3">
                    @foreach($relatedArtikels as $related)
                        <article class="overflow-hidden transition-all duration-300 bg-white shadow-lg dark:bg-gray-800 rounded-2xl hover:shadow-2xl hover:-translate-y-2">
                            ...
                            <div class="p-6">
                                ...
                                <a href="{{ route('artikel.show', $related->slug) }}" class="text-sm font-medium text-primary hover:underline">
                                    Baca →
                                </a>
                            </div>
                        </article>
    ...
    {{-- Back to Articles --}}
    <div class="container px-4 py-8 mx-auto md:px-32">
        <div class="text-center">
            <a href="{{ route('artikel.index') }}" class="inline-flex items-center gap-2 px-8 py-4 font-medium text-white transition-transform bg-primary rounded-xl hover:scale-105">
